...
//Modified to produce required data only for us.
//#include <cstdlib> 
#include <iostream> 
#include <fstream>
#include <assert.h>
...
int main(int argc, char *argv[])
{
  ...
  ofstream outputfile;
  string outputfile_name = trace_path + ".txt";
  cout << "Saving to" << outputfile_name << endl;
  ...
  for (auto it = bt9_reader.begin(); it != bt9_reader.end(); ++it)
  {
    ...
    try
    {
      ...
      outputfile << (UINT16)opType << ", " << PC << ", " << branchTarget << ", " << (int)branchTaken << endl;
